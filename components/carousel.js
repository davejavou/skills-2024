...
export default function Carousel({slides, sliderRef}) {
  ...
  return (
        ...
        <div key={sliderRef}>
          {(slides.length < 2) ?
            ...
            :
            <Slider {...sliderSettings} className="max-w-[1280px] mx-auto">
              {slides.map((slide, slideIndex) =>
                <div key={`${sliderRef}-${slideIndex}`}> {/* This container div will be styled by react-slick carosuel. Do not style or add classes */}
                  {slideContent(slide)}
                </div>
              )}
            </Slider>
